...
def update(request: schemas.User, db: Session):
    ...
    if (helpers.checkEmail(request['email'])):
        ...
        if exists:
            ...
            existing_user = models.User(firstname=request['firstname'], lastname=request['lastname'],
                                        orc_id=request['orc_id'], scopus_id=request['scopus_id'], email=request['email'], uid=request['uid'])
            scopusList = []
            orcidList = []
            ...
            if papersToAdd:
                ...
                for index in range(len(papersToAdd)):
                    ...
                    new_paper.__setattr__('creators', exists)
                    tempPapers.append(new_paper)
                    db.add(new_paper)
                    db.commit()
                    db.refresh(new_paper)
            ...
            raise HTTPException(
                status_code=status.HTTP_302_FOUND, detail="This user does not exist.")
    else:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Please use a valid email address.")
